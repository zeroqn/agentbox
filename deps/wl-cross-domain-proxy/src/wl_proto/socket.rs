...
impl BufferedSocket {
    ...
    // in the buffer
    fn attempt_write_message(&mut self, msg: &Message<u32, OwnedFd>) -> IoResult<bool> {
        match write_to_buffers(msg, self.out_data.get_writable_storage(), &mut self.out_fds) {
            ...
            Err(MessageWriteError::DupFdFailed) => Err(ErrorKind::Unsupported.into()),
        }
    }
    ...
    /// will be returned.
    pub fn write_message(&mut self, msg: &Message<u32, OwnedFd>) -> IoResult<()> {
...
impl<T: Copy + Default> Buffer<T> {
    ...
    pub fn get_writable_storage(&mut self) -> &mut [T] {
...
#[cfg(test)]
mod tests {
    use super::super::protocol::{AllowNull, Argument, ArgumentType, Message};
    use super::*;

...
use super::protocol::{Argument, ArgumentType, Message};
...
pub fn write_to_buffers<F: TryClone>(
    msg: &Message<u32, F>,
    payload: &mut [u8],
    fds: &mut Vec<F>,
    ...
    for arg in &msg.args {
        payload = match *arg {
            ...
            Argument::Fd(ref fd) => {
                let dup_fd = fd.try_clone().ok_or(MessageWriteError::DupFdFailed)?;
                fds.push(dup_fd);
                payload
            }
        };
...
pub fn parse_message<'a, F>(
    raw: &'a [u8],
    signature: &[ArgumentType],
    fds: &mut VecDeque<F>,
) -> Result<(Message<u32, F>, &'a [u8]), MessageParseError> {
    ...
    let fd_len = signature
        .iter()
        .filter(|x| matches!(x, ArgumentType::Fd))
        .count();
    if fd_len > fds.len() {
...
#[cfg(test)]
mod tests {
    use super::super::protocol::AllowNull;
    use super::*;
    use smallvec::smallvec;
    use std::ffi::CString;
    use std::os::fd::RawFd;

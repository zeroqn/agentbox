...
out_dir=""
cang_bin="${CANG_BIN:-$repo_root/result/bin/cang}"
use_cargo_run=0
skip_live=0
live_failed=0
rmux_bin="${RMUX_BIN:-}"
...
live_state_home="${CANG_BENCH_XDG_STATE_HOME:-}"
live_guest_init="${CANG_BENCH_GUEST_INIT:-}"
default_live_guest_init=1
require_guest_profile=0

usage() {
  cat <<'USAGE'
Usage: cang-pty-benchmark.sh [OPTIONS]

Run reproducible PTY benchmark diagnostics for cang attach-loop latency. The
runner records synthetic PTY baselines, a required live cang run with
CANG_ATTACH_PROFILE=1, and optional rmux/tmux comparison hooks when available.

Options:
      --cang <path>          cang binary to run (default: $CANG_BIN or ./result/bin/cang)
      --cang-cargo-run       run cang as: cargo run -p cang -- (explicit opt-in)
      --out-dir <path>        output directory (default: .omx/benchmarks/cang-pty/<timestamp>)
      ...
      --skip-live             skip required live cang run (for synthetic development smoke only)
      --timeout <seconds>     live command timeout (default: 180)
      --cang-arg <arg>       repeatable extra argument passed before the guest command
      --no-default-live-mem  do not add the benchmark default --mem 2 live-run arg
      --state-home <path>    XDG_STATE_HOME for live cang (default: btrfs containers disk when needed)
      --guest-init <path>    guest init path for live cang (default: ./result/bin/cang-guest-init if present)
...
Completion evidence for cang PTY work should not use --skip-live: live runs
must capture the host "cang attach profile" summary. Guest summaries are
captured when the guest/libkrun console is visible; use --require-guest-profile
for strict guest-profile diagnostics.

The default live run remains the single live-cang-shell smoke/profile capture.
Pass --live-iterations <n> to add the live-cang-redraw-typing scenario. By
default those measured samples are driven through one persistent live cang
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --cang) cang_bin="${2:?missing value for --cang}"; use_cargo_run=0; shift 2 ;;
    --cang-cargo-run) use_cargo_run=1; shift ;;
    ...
    --cang-arg) cang_extra_args+=("${2:?missing value for --cang-arg}"); shift 2 ;;
...
if [ -z "$out_dir" ]; then
  out_dir="$repo_root/.omx/benchmarks/cang-pty/$(date -u +%Y%m%dT%H%M%SZ)"
fi
...
if [ "$use_cargo_run" -eq 1 ]; then
  command -v cargo >/dev/null 2>&1 || { echo "--cang-cargo-run requires cargo" >&2; exit 1; }
  cang_display="cargo run -p cang --"
elif [ -x "$cang_bin" ]; then
  cang_display="$cang_bin"
else
  echo "cang binary is not executable: $cang_bin" >&2
  echo "pass --cang <path>, set CANG_BIN, or use --cang-cargo-run" >&2
  exit 1
fi
...
def cat_payload():
    return b"cang-pty-benchmark-cat-echo-0123456789abcdef\n" * 128

def redraw_payload():
    return b"".join(f"\x1b[2K\rcang redraw frame {i:03d} cursor-snap probe".encode() for i in range(160)) + b"\n"
...
default_live_state_home() {
  ...
  if [ -d "$containers_root" ] && [ -w "$containers_root" ]; then
    ...
    printf '%s\n' "$containers_root/cang-pty-benchmark-state/$slug"
    return 0
  fi
...
cang_args_contain_mem() {
  local arg
  for arg in "${cang_extra_args[@]}"; do
    case "$arg" in
      --mem|--mem=*) return 0 ;;
    esac
  done
...
default_live_guest_init_path() {
  ...
  if [ "$default_live_guest_init" -eq 1 ] && [ -x "$repo_root/result/bin/cang-guest-init" ]; then
    printf '%s\n' "$repo_root/result/bin/cang-guest-init"
    return 0
  fi
  return 1
}
...
append_default_live_args() {
  if [ -n "$default_live_mem_gib" ] && ! cang_args_contain_mem; then
    printf '%s\0%s\0' --mem "$default_live_mem_gib"
  fi
  if ! cang_args_contain_guest_init; then
...
def run_sample(ordinal, measured_iteration):
    ...
    label = f"cang-pty-bench-{os.getpid()}-{ordinal}-{time.time_ns()}"
    session = "bench"
...
run_live_cang() {
  local stdout_path="$out_dir/logs/live-cang-shell.stdout"
  local stderr_path="$out_dir/logs/live-cang-shell.stderr"
  local status_path="$out_dir/logs/live-cang-shell.status"
  local -a command_prefix
  local -a effective_cang_args
  local -a default_args
  effective_cang_args=("${cang_extra_args[@]}")
  mapfile -d '' -t default_args < <(append_default_live_args)
  if [ "${#default_args[@]}" -gt 0 ]; then
    effective_cang_args=("${default_args[@]}" "${effective_cang_args[@]}")
  fi
  if [ "$use_cargo_run" -eq 1 ]; then
    command_prefix=(cargo run -p cang --)
  else
    command_prefix=("$cang_bin")
  fi
...
i=0
while [ "$i" -lt 160 ]; do
  printf '\033[2K\rcang-live-pty-benchmark-%03d cursor-snap-probe' "$i"
  i=$((i + 1))
done
printf '\n'
SH
  ...
  local -a full_command=("${command_prefix[@]}" "${effective_cang_args[@]}" -- bash "$workload_guest_path")
  local cang_effective_display="$cang_display"
  if [ "${#effective_cang_args[@]}" -gt 0 ]; then
    printf -v cang_effective_display '%q ' "$cang_display" "${effective_cang_args[@]}"
    cang_effective_display="${cang_effective_display% }"
  fi
  ...
  python3 - "$metrics_path" "$stdout_path" "$stderr_path" "$status_path" "$timeout_seconds" "$cang_effective_display" "$effective_state_home" "$require_guest_profile" -- "${full_command[@]}" <<'PY'
import fcntl, json, os, pathlib, pty, re, selectors, signal, struct, sys, termios, time
metrics_path, stdout_path, stderr_path, status_path, timeout_seconds, cang_display, state_home, require_guest_profile = sys.argv[1:9]
command = sys.argv[10:]
...
roles = []
for match in re.finditer(r"cang attach profile role=(host|guest)([^\r\n]*)", combined):
    role = match.group(1)
    profile = {"role": role}
...
with open(metrics_path, "a", encoding="utf-8") as metrics:
    base = {
        "schema_version": 1, "scenario": "live-cang-shell", "mode": "live_cang", "iteration": 0,
        ...
        "cang_command": cang_display,
        "env": {"CANG_ATTACH_PROFILE": "1", "XDG_STATE_HOME": state_home or env.get("XDG_STATE_HOME")},
    }
    if roles:
        for role, profile in roles:
            record = dict(base)
            record.update({"status": "ok" if exit_status == 0 else "failed", "profile_role": role, "profile": profile, "error": None if exit_status == 0 else f"cang exited with status {exit_status}"})
            metrics.write(json.dumps(record, sort_keys=True) + "\n")
    else:
        reason = "timed out" if timed_out else "missing cang attach profile summaries"
        record = dict(base)
        record.update({"status": "failed", "profile_role": None, "profile": {}, "error": f"{reason}; cang exit status {exit_status}"})
        metrics.write(json.dumps(record, sort_keys=True) + "\n")
required_roles = {"host"}
...
if exit_status != 0:
    raise SystemExit(f"live cang failed with status {exit_status}; see {stdout_path}")
if missing:
    raise SystemExit(f"missing live cang profile role(s): {', '.join(sorted(missing))}; see {stdout_path}")
if not require_guest_profile and "guest" not in {role for role, _ in roles}:
    print(
        "warning: live run did not capture a guest profile; pass --cang-arg --log-level "
        "--cang-arg debug plus --require-guest-profile for strict guest diagnostics",
        file=sys.stderr,
    )
PY
}

run_live_cang_redraw_typing() {
  if [ "$live_iterations" -eq 0 ]; then
    append_skip "live-cang-redraw-typing" "live_cang_redraw_typing" "--live-iterations is 0; opt-in live interactive scenario not requested"
    return 0
  fi
...
while [ "$sample_index" -lt "$sample_count" ]; do
  ...
  printf 'CANG_PTY_READY_%s\n' "$sample"
  i=0
  while [ "$i" -lt "$marker_count" ]; do
    if ! IFS= read -r line; then
      break
    fi
    expected="CANG_PTY_INPUT_${sample}_${i}"
    if [ "$line" != "$expected" ]; then
      printf 'CANG_PTY_UNEXPECTED_%s_%d\n' "$sample" "$i"
      continue
    fi
    frame=0
    while [ "$frame" -lt "$redraw_frames_per_marker" ]; do
      printf '\033[2K\rcang-live-redraw-typing-%s-%02d-%03d cursor-snap-probe' "$sample" "$i" "$frame"
      frame=$((frame + 1))
    done
    printf '\nCANG_PTY_OUTPUT_%s_%d\n' "$sample" "$i"
    i=$((i + 1))
  done
  printf 'CANG_PTY_DONE_%s\n' "$sample"
...
SH
  ...
  local -a full_command_base=("${command_prefix[@]}" "${effective_cang_args[@]}" -- bash "$workload_guest_path")
  python3 - "$metrics_path" "$out_dir" "$timeout_seconds" "$cang_effective_display" "$effective_state_home" "$require_guest_profile" "$live_iterations" "$live_warmup" "$marker_count" "$redraw_frames_per_marker" "$live_per_sample_vm" -- "${full_command_base[@]}" <<'PY'
import json
import os
import pathlib
import pty
...
def build_sample_state(session_id, internal_ordinal, measured_iteration):
    ...
    return {
        ...
        "sample": sample,
        "ready_token": f"CANG_PTY_READY_{sample}",
        "done_token": f"CANG_PTY_DONE_{sample}",
        "output_tokens": [f"CANG_PTY_OUTPUT_{sample}_{idx}" for idx in range(marker_count)],
        "input_tokens": [f"CANG_PTY_INPUT_{sample}_{idx}" for idx in range(marker_count)],
        "hot_started_ns": None,
...
def drive_session(session_id, total_samples, measured_iteration_for_internal, measurement_model):
    ...
    stdout_path = logs_dir / (
        f"live-cang-redraw-typing-persistent-{session_id}.stdout"
        if persistent_session
        else f"live-cang-redraw-typing-{states[0]['measured_iteration']}.stdout"
    )
    stderr_path = logs_dir / (
        f"live-cang-redraw-typing-persistent-{session_id}.stderr"
        if persistent_session
        else f"live-cang-redraw-typing-{states[0]['measured_iteration']}.stderr"
    )
    status_path = logs_dir / (
        f"live-cang-redraw-typing-persistent-{session_id}.status"
        if persistent_session
        else f"live-cang-redraw-typing-{states[0]['measured_iteration']}.status"
    )
    ...
    for state in states:
        ...
        if error is None and exit_status != 0:
            error = f"cang exited with status {exit_status}"
        ...
        if error is None and "host" not in role_profiles:
            error = "missing cang attach profile role(s): host"
        if error is None and require_guest_profile and "guest" not in role_profiles:
            error = "missing cang attach profile role(s): guest"
        ...
        record = {
            "schema_version": 1,
            "scenario": "live-cang-redraw-typing",
            "mode": "live_cang_redraw_typing",
...
if [ "$skip_live" -eq 1 ]; then
  append_skip "live-cang-shell" "skip" "--skip-live was passed; live cang evidence not collected"
  append_skip "live-cang-redraw-typing" "skip" "--skip-live was passed; live cang evidence not collected"
else
  run_live_cang || live_failed=$?
  if [ "$live_failed" -eq 0 ]; then
    run_live_cang_redraw_typing || live_failed=$?
...
cang_summary_display="$cang_display"
summary_effective_cang_args=("${cang_extra_args[@]}")
summary_default_cang_args=()
mapfile -d '' -t summary_default_cang_args < <(append_default_live_args)
if [ "${#summary_default_cang_args[@]}" -gt 0 ]; then
  summary_effective_cang_args=("${summary_default_cang_args[@]}" "${summary_effective_cang_args[@]}")
fi
if [ "${#summary_effective_cang_args[@]}" -gt 0 ]; then
  printf -v cang_summary_display '%q ' "$cang_display" "${summary_effective_cang_args[@]}"
  cang_summary_display="${cang_summary_display% }"
fi

summary_state_home="$(default_live_state_home || true)"
python3 - "$metrics_path" "$summary_path" "$out_dir" "$cang_summary_display" "$skip_live" "$iterations" "$warmup" "$live_iterations" "$live_warmup" "$summary_state_home" "$require_guest_profile" "$live_per_sample_vm" <<'PY'
...
redraw_typing_profiles = [
    record.get("profile", {})
    for record in by_scenario.get("live-cang-redraw-typing", [])
...
if redraw_typing_profiles:
    ...
    scenario_profiles["live-cang-redraw-typing"] = {
...
summary = {
    ...
    "live_env": {"CANG_ATTACH_PROFILE": "1", "XDG_STATE_HOME": state_home or None},
...
print(f"cang PTY benchmark artifacts: {out_dir}")
...
if scenario_profiles.get("live-cang-redraw-typing"):
    live_profile = scenario_profiles["live-cang-redraw-typing"]
    print(

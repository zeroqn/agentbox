...
usage() {
  cat <<'EOF'
Usage: update-herdr.sh [--tag <release-tag>]

Refresh the pinned herdr prebuilt release metadata in nix/pins.nix by querying
...
if [ -z "$release_tag" ]; then
  echo "failed to determine latest herdr release tag; pass --tag explicitly" >&2
  exit 1
fi
...
if count != 1:
    raise SystemExit("failed to replace herdrPrebuiltRelease block; expected exactly one match")

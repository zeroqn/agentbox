...
`krun_start_enter`; `helper_wait_vm_worker_child_unattributed` covers any
remaining wait time outside the known child setup phases, usually guest/libkrun
runtime after the handoff. `--profile` does not raise loftd, guest-init, or
libkrun debug logging;

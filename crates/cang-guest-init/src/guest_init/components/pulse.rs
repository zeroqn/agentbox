...
const SOCKET_NAME: &str = "cang-pulse";
            ...
            Ok(client) => {
                thread::spawn(move || {
                    if let Err(err) = relay(client, port) {
                        eprintln!("cang-guest-init: Pulse bridge connection failed: {err:#}");
                    }
                });
            }
...
mod tests {
    ...
    fn server_value_uses_dev_runtime_directory() {
        ...
        assert_eq!(server_value(&identity), "unix:/run/user/1234/cang-pulse");
    }
}

...
fn preferred_candidate_match_returns_without_generic_discovery() {
    let probe = FakeDiskProbe::default().with_candidate_label("/dev/vda", Ok(Some("CANG_NIX")));

    let path = find_labeled_disk_with_probe("CANG_NIX", "cang-nix", &["/dev/vda"], &probe)
...
fn preferred_candidate_mismatch_falls_back_to_blkid_label_lookup() {
    let probe = FakeDiskProbe::default()
        ...
        .with_label_lookup(Ok(Some("/dev/disk/by-label/CANG_NIX")));

    let path = find_labeled_disk_with_probe("CANG_NIX", "cang-nix", &["/dev/vda"], &probe)
        .expect("label lookup fallback should be returned");

    assert_eq!(path, PathBuf::from("/dev/disk/by-label/CANG_NIX"));
    assert_eq!(
        probe.calls(),
        vec![
            ProbeCall::CandidateLabel("/dev/vda".to_owned()),
            ProbeCall::LookupLabel("CANG_NIX".to_owned())
        ]
    );
}
...
fn preferred_candidate_without_label_falls_back_to_enumeration() {
    ...
    assert_eq!(
        ...
        vec![
            ProbeCall::CandidateLabel("/dev/vda".to_owned()),
            ProbeCall::LookupLabel("CANG_NIX".to_owned()),
            ProbeCall::EnumerateCandidates("/dev/disk/by-id/*cang-nix*".to_owned()),
...
fn preferred_candidate_blkid_execution_error_is_not_silently_ignored() {
    ...
    let error = find_labeled_disk_with_probe("CANG_NIX", "cang-nix", &["/dev/vda"], &probe)
...
fn compatibility_entrypoint_starts_with_blkid_label_lookup_when_no_preferred_candidates() {
    let probe = FakeDiskProbe::default().with_label_lookup(Ok(Some("/dev/disk/by-label/CANG_NIX")));

    let path = find_labeled_disk_with_probe("CANG_NIX", "cang-nix", &[], &probe)
        .expect("label lookup should be returned");

    assert_eq!(path, PathBuf::from("/dev/disk/by-label/CANG_NIX"));
    assert_eq!(
        probe.calls(),
        vec![ProbeCall::LookupLabel("CANG_NIX".to_owned())]
    );
}

...
pub(in crate::guest_init) const REAL_PODMAN_ENV: &str = "CANG_REAL_PODMAN";
...
fn validate_real_podman_candidate(path: &Path) -> Result<()> {
    ...
    if looks_like_cang_wrapper(path)? {
        bail!(
...
fn looks_like_cang_wrapper(path: &Path) -> Result<bool> {
    ...
    Ok(text.contains("cang-guest-init internal podman "))
}

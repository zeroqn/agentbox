...
fn shell_config_materialization_installs_fish_starship_payloads() {
    ...
    let fish_source = source_dir.join("cang-starship.fish");
    ...
    assert_eq!(
        fs::read_to_string(identity.home.join(".config/fish/conf.d/cang-starship.fish"))
...
fn shell_config_materialization_preserves_existing_user_config() {
    ...
    fs::write(
        identity.home.join(".config/fish/conf.d/cang-starship.fish"),
        "custom fish\n",
    )

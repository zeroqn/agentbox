...
pub(in crate::guest_init) const WRAPPER_BIN_DIR: &str = "/run/cang/wrappers/bin";
...
fn install_granted_helper(permissions: GuestPermissions) -> Result<()> {
    ...
    let dst = wrapper_dir.join("cang-granted");
    let src = source_helper_on_path("cang-granted", wrapper_dir)?;
    command::run(
        "install",
        &[
            "-m",
    ...
    .context("failed to install root-owned cang-granted helper")?;
...
fn set_file_capabilities(path: &Path, capabilities: &[u32]) -> Result<()> {
    ...
    } != 0
    {
        return Err(std::io::Error::last_os_error())
            .context("failed to set cang-granted file capabilities");
    }
    Ok(())
}
...
fn verify_granted_helper(path: &Path, capabilities: &[u32]) -> Result<()> {
    ...
    {
        bail!("unexpected cang-granted ownership or mode");
    }
    ...
    if size < 0 {
        return Err(std::io::Error::last_os_error())
            .context("failed to verify cang-granted file capabilities");
    }

    let expected = file_capability_value(capabilities);
    if size != expected.len() as isize || value[..expected.len()] != expected {
        bail!("installed cang-granted capabilities differ from the authorized set");
    }
    Ok(())
}

fn seal_wrapper_tree() -> Result<()> {
    let wrappers = std::ffi::CString::new("/run/cang/wrappers")?;
    if unsafe {
        libc::mount(
            wrappers.as_ptr(),
            wrappers.as_ptr(),
    ...
    {
        return Err(std::io::Error::last_os_error()).context("failed to bind-mount cang wrappers");
    }
    if unsafe {
        libc::mount(
            std::ptr::null(),
    ...
    {
        return Err(std::io::Error::last_os_error())
            .context("failed to remount cang wrappers read-only");
    }
    Ok(())
}

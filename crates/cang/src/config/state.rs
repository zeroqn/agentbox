...
const APP_DIR_NAME: &str = "cang";
const CONFIG_FILE_NAME: &str = "cang.toml";
...
fn read_config_from_path(config_path: &Path) -> Result<CangConfig> {
    if !config_path.exists() {
        return Ok(CangConfig::missing(config_path.to_path_buf()));
    }
...
mod tests {
    ...
    fn default_config_path_prefers_xdg_config_home() {
        ...
        assert_eq!(path, Path::new("/tmp/xdg-config/cang/cang.toml"));
    }
    ...
    fn default_config_path_falls_back_to_home_config() {
        ...
        assert_eq!(path, Path::new("/tmp/home/.config/cang/cang.toml"));
    }
    ...
    fn read_config_reports_missing_file_diagnostics() {
        ...
        assert_eq!(config.path(), &dir.path().join("cang").join("cang.toml"));

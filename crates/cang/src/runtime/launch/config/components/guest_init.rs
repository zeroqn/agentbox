...
pub(crate) fn validate_guest_init_override_mount(
    ...
    if !mount.source.is_absolute() {
        anyhow::bail!(
            "cang guest-init override bind source '{}' must be absolute",
            mount.source.display()
        );
    }
    if !Path::new(&mount.target).is_absolute() {
        anyhow::bail!(
            "cang guest-init override bind target '{}' must be absolute",
            mount.target
        );
    }
    if !mount.read_only {
        anyhow::bail!("cang guest-init override bind must be read-only");
    }
    if mount.target != exec_path {
        anyhow::bail!(
            "cang guest-init override bind target '{}' must match guest-init exec path '{}'",
            mount.target,
            exec_path
        );
    }

...
use super::model::{IMAGE_CANG_ENV_ALLOWLIST, IMAGE_PATH_ENV};
...
pub(crate) fn bootstrap_env(
    ...
    for entry in image_env {
        let (key, value) = entry
            .split_once('=')
            .ok_or_else(|| anyhow!("cang image env entry '{entry}' is missing '='"))?;
        if key.is_empty() {
            anyhow::bail!("cang image env entry '{entry}' has an empty key");
        }
...
fn is_allowed_image_env(key: &str) -> bool {
    key == IMAGE_PATH_ENV || IMAGE_CANG_ENV_ALLOWLIST.contains(&key)
}

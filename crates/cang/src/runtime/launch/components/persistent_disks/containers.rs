...
pub(super) const FILE_NAME: &str = "cang-containers.raw";
pub(super) const ID: &str = "cang-containers";
pub(super) const LABEL: &str = "CANG_CONTAINERS";
pub(super) const SIZE_BYTES: u64 = 64 * 1024 * 1024 * 1024;
pub(super) const STORAGE_ENV: &str = "CANG_CONTAINERS_STORAGE";
pub(super) const STORE_ENV: &str = "CANG_CONTAINERS_STORE";
...
const SPEC: RawDiskSpec = RawDiskSpec {
    ...
    size_hint: "64G",
    diagnostic_name: "cang persistent container-store dev cache disk",
};
...
pub(super) fn prepare_with_runner(
    ...
    raw_btrfs::prepare_with_runner(state_root, &SPEC, runner)
        .context("failed to prepare cang persistent container-store dev cache disk")
}
...
pub(crate) fn grow_with_runner(
    ...
    raw_btrfs::grow_existing_with_runner(state_root, &SPEC, target_size_bytes, runner)
        .context("failed to grow cang persistent container-store dev cache disk")
}
...
pub(crate) fn recreate_with_runner(
    ...
    raw_btrfs::recreate_with_runner(state_root, &SPEC, runner)
        .context("failed to reset cang persistent container-store dev cache disk")
}
...
pub(super) fn raw_disk_env_pairs(disk: &RawBtrfsDisk) -> [(String, String); 4] {
    [
        ...
        ("CANG_CONTAINERS_DISK_ID".to_owned(), disk.id.clone()),
        ("CANG_CONTAINERS_DISK_LABEL".to_owned(), disk.label.clone()),
    ]
}

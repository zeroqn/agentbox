...
use cang_exec_protocol::{Frame, PROTOCOL_VERSION, WaypipeAction, read_frame, write_frame};
...
pub(crate) fn exec_in_task(
    ...
    if argv.is_empty() {
        bail!("cang exec requires a command");
    }
    ...
    match inspector.status(&record.process) {
        ...
        status => bail!(
            "cang task '{}' cannot execute commands because it is {}",
            record.task_id,
            status.as_str()
        ),
    }
    let exec = record.exec.as_ref().ok_or_else(|| {
        anyhow!(
            "cang task '{}' does not support exec; relaunch it with a current cang version",
            record.task_id
        )
    })?;
    if exec.protocol_version != PROTOCOL_VERSION {
        bail!(
            "cang exec protocol version mismatch: host supports {PROTOCOL_VERSION}, task uses {}",
    ...
    let waypipe_action = match waypipe {
        ...
        Some(None) => {
            if record.waypipe.is_none() {
                bail!(
                    "cang task '{}' does not support Waypipe; relaunch it with --waypipe",
                    record.task_id
                );
            }
            WaypipeAction::Reuse
...
fn exec_on_socket(socket: &Path, argv: Vec<String>, waypipe: WaypipeAction) -> Result<ExitCode> {
    let mut stream = UnixStream::connect(socket).with_context(|| {
        format!(
            "failed to connect to cang exec socket '{}'",
            socket.display()
        )
    })?;
    write_frame(
    ...
    match read_frame(&mut stream)? {
        ...
        Some(frame) => bail!("unexpected cang exec handshake frame: {frame:?}"),
        None => bail!("cang exec connection closed during handshake"),
    }
    ...
    loop {
        ...
        if rc < 0 {
            ...
            return Err(err).context("cang exec poll failed");
        }
        ...
        if fds[0].revents & (libc::POLLIN | libc::POLLHUP | libc::POLLERR) != 0 {
            match read_frame(&mut stream)? {
                ...
                Some(frame) => return Err(anyhow!("unexpected cang exec frame: {frame:?}")),
                None => return Err(anyhow!("cang exec connection closed before exit status")),
            }
        }
        ...
        if stdin_open && fds[1].revents & (libc::POLLIN | libc::POLLHUP | libc::POLLERR) != 0 {
            match stdin.lock().read(&mut input) {
                ...
                Err(err) => return Err(err).context("failed to read cang exec stdin"),
            }
        }
    ...
    fn block() -> Result<Self> {
        ...
        for signal in FORWARDED_SIGNALS {
            if unsafe { libc::sigaddset(&mut set, signal) } != 0 {
                return Err(std::io::Error::last_os_error())
                    .context("failed to add cang exec signal to signal set");
            }
        }
        ...
        if rc != 0 {
            return Err(std::io::Error::from_raw_os_error(rc))
                .context("failed to block cang exec signals");
        }
        Ok(Self {
            set,
...
impl SignalFd {
    fn new(set: &libc::sigset_t) -> Result<Self> {
        ...
        if fd < 0 {
            return Err(std::io::Error::last_os_error())
                .context("failed to create cang exec signal fd");
        }
        Ok(Self(fd))
    }
    ...
    fn read_signal(&self) -> Result<i32> {
        ...
        if read != std::mem::size_of::<libc::signalfd_siginfo>() as isize {
            return Err(std::io::Error::last_os_error())
                .context("failed to read cang exec signal fd");
        }

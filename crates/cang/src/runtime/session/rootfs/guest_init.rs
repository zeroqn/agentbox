...
pub(crate) fn resolve_guest_init_with_entrypoint(
    ...
    let target = find_cang_guest_init(task_rootfs)?;
    ...
    match image_entrypoint {
        [exec_path, enter, separator] if enter == "enter" && separator == "--" => {
            ...
            if !guest_path.is_absolute() {
                bail!(
                    "cang image entrypoint '{}' is not absolute; direct-libkrun requires an absolute {GUEST_INIT_BASENAME} entrypoint",
                    exec_path
                );
            }
            if guest_path.file_name().and_then(|name| name.to_str()) != Some(GUEST_INIT_BASENAME) {
                bail!(
                    "cang image entrypoint '{}' does not point to {GUEST_INIT_BASENAME}",
                    exec_path
                );
            }
            ...
            if !is_executable_file(&host_path) {
                bail!(
                    "cang image entrypoint '{}' does not resolve to an executable file in task rootfs '{}'",
                    exec_path,
                    task_rootfs.display()
                );
            }
        ...
        [_, enter, separator, extra @ ..] if enter == "enter" && separator == "--" => {
            bail!(
                "cang image entrypoint has unsupported extra args after 'enter --': {:?}",
                extra
            )
        }
        _ => bail!(
            "cang image entrypoint is not compatible with direct-libkrun; expected [*/{GUEST_INIT_BASENAME}, \"enter\", \"--\"]"
        ),
    }
}

fn find_cang_guest_init(task_rootfs: &Path) -> Result<PathBuf> {
    let store = task_rootfs.join("nix/store");
    let mut matches = Vec::new();
    if store.is_dir() {
        for entry in fs::read_dir(&store).with_context(|| {
            format!(
                "failed to read cang task rootfs store '{}'",
                store.display()
            )
        })? {
            let entry = entry?;
    ...
    match matches.len() {
        ...
        0 => bail!(
            "cang image is not cang-compatible: no executable {GUEST_INIT_BASENAME} found under {}/nix/store/*/bin",
            task_rootfs.display()
        ),
        count => bail!(
...
fn validate_guest_init_override(override_path: &Path) -> Result<PathBuf> {
    let source = override_path.canonicalize().with_context(|| {
        format!(
            "failed to resolve cang guest-init override '{}'",
            override_path.display()
        )
    })?;
    if !is_executable_file(&source) {
        bail!(
            "cang guest-init override '{}' is not an executable regular file",
            source.display()
        );
    }
    Ok(source)
...
fn guest_path(task_rootfs: &Path, host_path: &Path) -> Result<String> {
    let relative = host_path.strip_prefix(task_rootfs).with_context(|| {
        format!(
            "cang guest-init target '{}' is outside task rootfs '{}'",
            host_path.display(),
            task_rootfs.display()
        )
    })?;
...
mod tests {
    ...
    fn resolves_exactly_one_executable_guest_init_from_task_rootfs() {
        ...
        let host_path = write_guest_init(temp.path(), "hash-cang");
    ...
    fn override_is_bound_read_only_over_task_rootfs_target_without_mutating_target() {
        let temp = tempfile::tempdir().expect("tempdir should be created");
        let target = write_guest_init(temp.path(), "hash-cang");
        let override_path = temp.path().join("override-cang-guest-init");
    ...
    fn compatible_image_entrypoint_is_used_as_guest_init_exec() {
        let temp = tempfile::tempdir().expect("tempdir should be created");
        write_guest_init(temp.path(), "hash-cang");

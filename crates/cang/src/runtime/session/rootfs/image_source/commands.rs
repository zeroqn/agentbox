    ...
    let rootfs = match (materialized, cleanup) {
        ...
        (Err(err), Err(cleanup_err)) => {
            return Err(cleanup_err.context(format!(
                "failed to clean cang image cache staging after sync error: {err:#}"
            )));
        }
    };
...
fn list_image_cache(
    ...
    if cache_dir.exists() {
        for dir_entry in fs::read_dir(&cache_dir)
            .with_context(|| format!("failed to read cang image cache '{}'", cache_dir.display()))?
        {
...
fn inspect_remove_image_cache_entry(
    ...
    if !entry.entry_dir.exists() {
        bail!(
            "cang image cache entry '{}' does not exist",
    ...
    match resolve_local_selector(image_cache_root, buildah, target)? {
        SelectorResolution::One(row) => {
            let digest_key = row.digest_key.as_deref().ok_or_else(|| {
                anyhow!("image selector '{target}' matched a row without a cang cache key")
            })?;
            let digest = digest_from_digest_key_path_component(digest_key).ok_or_else(|| {
                anyhow!("image selector '{target}' matched a row with an invalid cang cache key")
            })?;
        ...
        SelectorResolution::None => {
            bail!("image selector '{target}' did not match a cang image cache row")
        }
    }
}
...
fn read_buildah_inventory(buildah: &impl BuildahCommands) -> Vec<BuildahImageRow> {
    ...
    let output = match buildah.run(&[
        ...
        Ok(output) => output,
        Err(err) => {
            eprintln!("cang: buildah inventory failed: {err}");
            return Vec::new();
        }
    };
    output
...
fn read_cached_metadata_for_remove(
    ...
    let metadata = fs::read_to_string(&entry.metadata_path).with_context(|| {
        format!(
            "failed to read cang image cache metadata '{}'",
...
fn validate_digest_key_target(target: &str) -> Result<()> {
    ...
    {
        bail!("cang image cache digest key '{target}' is not safe");
    }
    Ok(())
}
...
impl ImageCacheStagingDir {
    fn create(image_cache_root: &Path) -> Result<Self> {
        ...
        fs::create_dir_all(&path).with_context(|| {
            format!(
                "failed to create cang image cache staging directory '{}'",
                path.display()
            )
        })?;
        Ok(Self { path })
    ...
    fn cleanup(self, btrfs: &impl BtrfsRootfsCommands) -> Result<()> {
        ...
        if rootfs_path.exists() {
            btrfs
                ...
                .with_context(|| {
                    format!(
                        "failed to delete cang image cache staging rootfs '{}'",
                        rootfs_path.display()
                    )
                })?;
        }
        if self.path.exists() {
            remove_cache_entry_tree(&self.path).with_context(|| {
                format!(
                    "failed to remove cang image cache staging directory '{}'",
                    self.path.display()
                )
            })?;
        }

...
impl<C: BtrfsRootfsCommands> Drop for TaskRootfsLease<C> {
    fn drop(&mut self) {
        ...
        if let Err(err) = cleanup_task_dir(&handle.task_dir, &self.commands) {
            eprintln!(
                "cang: best-effort task rootfs cleanup failed for '{}': {err:#}",
...
pub(crate) fn snapshot_mounted_rootfs(
    ...
    commands
        ...
        .with_context(|| {
            format!(
                "btrfs-snapshot backend requires Buildah storage and cang task state on snapshot-compatible btrfs subvolumes; failed to snapshot '{}' to '{}'",
                source.display(),
                destination.display()
            )
        })
...
fn cleanup_after_materialization_failure(
    ...
    if preserve_debug {
        return err.context(format!(
            "preserved partial cang task rootfs state at '{}'",
            task_dir.display()
        ));
    }
    match cleanup_task_dir(task_dir, commands) {
        ...
        Err(cleanup_err) => cleanup_err.context(format!(
            "failed to clean partial cang task rootfs after materialization error: {err:#}"
        )),
    }
}
...
fn reset_task_dir(task_dir: &Path, commands: &impl BtrfsRootfsCommands) -> Result<()> {
    if task_dir.exists() {
        cleanup_task_dir(task_dir, commands).with_context(|| {
            format!(
                "failed to reset stale cang task rootfs directory '{}'",
                task_dir.display()
            )
        })?;
    }
...
fn cleanup_task_dir(task_dir: &Path, commands: &impl BtrfsRootfsCommands) -> Result<()> {
    ...
    remove_task_rootfs_tree(task_dir).with_context(|| {
        format!(
            "failed to remove cang task rootfs directory '{}'",
            task_dir.display()
        )
    })
}
...
fn make_directories_owner_writable(path: &Path) -> Result<()> {
    ...
    if mode & 0o200 == 0 {
        fs::set_permissions(path, fs::Permissions::from_mode(mode | 0o700)).with_context(|| {
            format!(
                "failed to make cang task rootfs directory writable '{}'",
                path.display()
            )
        })?;
    }

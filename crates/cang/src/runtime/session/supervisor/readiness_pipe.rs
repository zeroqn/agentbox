...
pub(crate) const READY_FD_ENV: &str = "CANG_MANAGED_READY_FD";
    ...
    pub(crate) fn wait_for_ready(&mut self, child: &mut Child, timeout: Duration) -> Result<()> {
        ...
        loop {
            if let Some(status) = child
                .try_wait()
                .context("failed to poll cang helper while waiting for managed attach readiness")?
            {
                bail!("cang helper exited with {status} before managed attach socket was ready");
            }

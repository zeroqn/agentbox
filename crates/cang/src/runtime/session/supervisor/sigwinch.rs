...
impl SigwinchForwarder {
    pub(crate) fn start(target_pgid: u32) -> Result<Self> {
        ...
        let thread = thread::Builder::new()
            .name("cang-sigwinch".to_owned())
            .spawn(move || relay_sigwinch_events(target_pgid, relay_active))
            .context("failed to spawn cang SIGWINCH relay thread")?;
        Ok(Self {
            active,
...
impl BlockedSigwinchMask {
    fn block_current_thread() -> Result<Self> {
        if ACTIVE_SIGWINCH_BROKER
            ...
            .is_err()
        {
            bail!("cang SIGWINCH forwarding already has an active broker");
        }
        ...
        if rc != 0 {
            ...
            bail!(
                "failed to block SIGWINCH in cang supervisor thread: {}",
            ...
            Ok(SigwaitOutcome::Received) => {
                if let Err(err) = forward_sigwinch(target_pgid, &mut signaler) {
                    tracing::warn!(
                        pgid = target_pgid,
                        error = %err,
                        "cang SIGWINCH relay failed"
                    );
                }
            }
            Err(err) => {
                tracing::warn!(error = %err, "cang SIGWINCH wait failed");
                return;
            }
        }
    }

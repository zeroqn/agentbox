...
pub(crate) fn require_real_etc_dir(root_export: &Path) -> Result<PathBuf> {
    ...
    if metadata.file_type().is_symlink() || !metadata.is_dir() {
        bail!(
            "cang prepared root requires '{}' to be a real directory",
            etc_dir.display()
        );
    }
    Ok(etc_dir)
...
fn validate_replaceable_runtime_file(path: &Path) -> Result<()> {
    match fs::symlink_metadata(path) {
        ...
        Ok(_) => bail!(
            "cang runtime etc path '{}' must be a regular file or symlink",
            path.display()
        ),
...
mod tests {
    ...
    fn files() -> RuntimeEtcFiles {
        RuntimeEtcFiles {
            ...
            hosts: "127.0.0.1\tlocalhost cang-workspace\n::1\tlocalhost ip6-localhost ip6-loopback\n169.254.1.2\thost.containers.internal host.docker.internal\n"
                .to_owned(),
    ...
    fn hosts_file_always_contains_podman_like_host_aliases() {
        let hosts = hosts_file("cang-workspace");

        assert!(hosts.contains("127.0.0.1\tlocalhost cang-workspace\n"));

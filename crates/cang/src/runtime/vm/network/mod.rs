...
impl NetworkManagerSession {
    pub(crate) fn start(
        ...
        fs::create_dir_all(task_state_dir).with_context(|| {
            format!(
                "failed to create cang network task state dir '{}'",
        ...
        wait_for_stable_child(&mut pasta).with_context(|| {
            format!(
                "pasta failed to initialize cang network namespace '{}'",
                plan.args.join(" ")
            )
        })?;
...
impl PasstWorkerSession {
    pub(crate) fn start(publish: &[String]) -> Result<Self> {
        ...
        wait_for_stable_child(&mut child).with_context(|| {
            format!(
                "passt failed to initialize cang fd backend '{}'",
                plan.args.join(" ")
            )
        })?;
...
fn passt_socketpair() -> Result<(OwnedFd, OwnedFd)> {
    ...
    if rc < 0 {
        bail!(
            "failed to create cang passt socketpair: {}",
...
impl ManagedChild {
    fn spawn(mut command: Command, name: &'static str) -> Result<Self> {
        let child = command
            .spawn()
            .with_context(|| format!("failed to start cang {name}; is {name} on PATH?"))?;
        Ok(Self { child, name })
    }
    ...
    fn has_exited(&mut self) -> Result<Option<std::process::ExitStatus>> {
        self.child
            .try_wait()
            .with_context(|| format!("failed to poll cang {}", self.name))
    }
...
pub(crate) fn enter_netns(holder_pid: libc::pid_t) -> Result<()> {
    ...
    if fd < 0 {
        bail!(
            "failed to open cang target network namespace '{}': {}",
            path,
            std::io::Error::last_os_error()
        );
    }
    ...
    if rc < 0 {
        bail!(
            "failed to enter cang target network namespace '{}': {}",
            path,
            std::io::Error::last_os_error()
        );
    }
...
fn ensure_executable_available(program: OsString, name: &str) -> Result<()> {
    let status = Command::new(&program)
        ...
        .with_context(|| {
            format!(
                "cang requires {name} for host alias networking; resolved helper path was {}",
    ...
    if status.success() {
        ...
        bail!(
            "cang requires executable {name}, but `{}` --help failed with {status}",
            display_program(&program)
        )
    }
}
...
fn spawn_netns_holder() -> Result<HolderGuard> {
    let (read_fd, write_fd) = pid::pipe_cloexec("cang network namespace holder readiness pipe")?;
    ...
    if pid < 0 {
        bail!(
            "failed to fork cang network namespace holder: {}",
            std::io::Error::last_os_error()
        );
    }
    if pid == 0 {
...
fn wait_for_holder_ready(fd: OwnedFd, pid: libc::pid_t) -> Result<()> {
    ...
    match file.read(&mut byte) {
        ...
        Ok(_) => {
            ...
            bail!("cang network namespace holder exited before readiness")
        }
        Err(err) => Err(err).context("failed to read cang network namespace holder readiness"),
    }
}
...
fn wait_for_stable_child(child: &mut ManagedChild) -> Result<()> {
    ...
    while Instant::now() < deadline {
        if let Some(status) = child.has_exited()? {
            bail!("cang {} exited before readiness with {status}", child.name);
        }

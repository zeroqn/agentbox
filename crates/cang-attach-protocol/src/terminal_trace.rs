...
const TERMINAL_TRACE_ENV: &str = "CANG_TERMINAL_TRACE";
pub const TERMINAL_TRACE_FILE_NAME: &str = "cang-terminal.trace";
pub const GUEST_TERMINAL_TRACE_PATH: &str = "/workspace/cang-terminal.trace";
...
pub fn prepare_terminal_trace_file_from_process_env(
    ...
    if cli_trace {
        // SAFETY: cang calls this during launch setup, before the managed-session
...
mod tests {
    ...
    fn trace_prepare_truncates_host_workspace_trace_file() {
        with_terminal_trace_env(Some("1"), || {
            let dir = unique_temp_dir("cang-terminal-trace-prepare");
    ...
    fn host_trace_writer_uses_current_dir_and_ignores_env_path() {
        with_terminal_trace_env(None, || {
            let dir = unique_temp_dir("cang-terminal-trace-host");
    ...
    fn trace_truncation_helper_truncates_existing_file() {
        let path = std::env::temp_dir().join(format!(
            "cang-terminal-trace-truncate-{}.log",
            unix_timestamp_micros()
        ));
        std::fs::write(&path, "sentinel").unwrap();

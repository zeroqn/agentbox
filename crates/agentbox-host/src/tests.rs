...
fn publish_image_workflows_publish_separate_loftd_and_agentbox_names() {
    for workflow in [PUBLISH_IMAGE_YML, PUBLISH_DEV_IMAGE_YML] {
        for required in [
            ...
            "mkdir -p .ci-sccache",
            "chmod -R a+rwX .ci-sccache",
            r#"--option extra-sandbox-paths "/nix/var/cache/sccache=${GITHUB_WORKSPACE}/.ci-sccache""#,
            r#"".#${{ matrix.flake_attr }}""#,
...
fn publish_release_uploads_agentbox_and_neutral_loftd_assets() {
    for required in [
        "uses: actions/cache@v4",
        "path: .ci-sccache",
        "sccache_sandbox_path=\"/nix/var/cache/sccache=${GITHUB_WORKSPACE}/.ci-sccache\"",

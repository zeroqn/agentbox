...
fn publish_image_workflows_publish_separate_loftd_and_agentbox_names() {
    for workflow in [PUBLISH_IMAGE_YML, PUBLISH_DEV_IMAGE_YML] {
        for required in [
            ...
            "include:",
            "image_name: agentbox",
            "flake_attr: agentbox-container-ci-sccache",
            ...
            "path: /tmp/agentbox-ci-sccache-${{ matrix.image_name }}",
            "hashFiles('Cargo.lock', 'flake.lock', 'nix/pins.nix')",
            r#"mkdir -p "/tmp/agentbox-ci-sccache-${{ matrix.image_name }}""#,
            r#"sudo chown -R "$(id -u):$(id -g)" "/tmp/agentbox-ci-sccache-${{ matrix.image_name }}""#,
            r#"chmod -R a+rwX "/tmp/agentbox-ci-sccache-${{ matrix.image_name }}""#,
            r#"--option extra-sandbox-paths "/nix/var/cache/sccache=/tmp/agentbox-ci-sccache-${{ matrix.image_name }}""#,
            r#"".#${{ matrix.flake_attr }}""#,
            r#"-o "result-${{ matrix.image_name }}-container""#,
            r#"--print-out-paths > "${{ matrix.image_name }}-container-path.txt""#,
            "if: always()",
...
fn publish_release_uploads_agentbox_and_neutral_loftd_assets() {
    for required in [
        "uses: actions/cache@v4",
        "path: /tmp/agentbox-ci-sccache-release",
        "mkdir -p /tmp/agentbox-ci-sccache-release",
        "sudo chown -R \"$(id -u):$(id -g)\" /tmp/agentbox-ci-sccache-release",
        "chmod -R a+rwX /tmp/agentbox-ci-sccache-release",
        "sccache_sandbox_path=\"/nix/var/cache/sccache=/tmp/agentbox-ci-sccache-release\"",
        "nix build --option extra-sandbox-paths \"${sccache_sandbox_path}\" .#agentbox-musl-ci-sccache -o result-agentbox-musl",

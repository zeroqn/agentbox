...
impl<A: LibkrunApi> DirectLibkrunLauncher<A> {
    ...
    pub(crate) fn start_enter(mut self, config: &MicrovmLaunchConfig) -> Result<()> {
        let ctx_id = self
            .api
            .create_ctx()
    ...
    fn configure_and_start(&mut self, ctx_id: u32, config: &MicrovmLaunchConfig) -> Result<()> {
...
mod tests {
    ...
    use crate::runtime::microvm::launch::MicrovmLaunchSpec;

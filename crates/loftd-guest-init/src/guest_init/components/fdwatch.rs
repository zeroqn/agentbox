...
//!
//! A "Too many open files" failure inside a loftd guest has two possible
...
/// Renders the `key=value` report written to `/run/loftd/fd-pressure.status`
/// and printed by `loftd-guest-init fd-report`.
...
pub(in crate::guest_init) fn record_and_warn(tick: &FdWatchTick) {
    ...
    for warning in &tick.warnings {
        eprintln!("loftd-guest-init: fd pressure: {}", warning.render());
    }
}

/// `loftd-guest-init fd-report` implementation.

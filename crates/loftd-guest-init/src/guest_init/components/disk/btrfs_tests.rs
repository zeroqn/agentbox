...
fn preferred_candidate_match_returns_without_generic_discovery() {
    let probe = FakeDiskProbe::default().with_candidate_label("/dev/vda", Ok(Some("LOFTD_NIX")));

    let path = find_labeled_disk_with_probe("LOFTD_NIX", "loftd-nix", &["/dev/vda"], &probe)
...
fn preferred_candidate_mismatch_falls_back_to_blkid_label_lookup() {
    let probe = FakeDiskProbe::default()
        ...
        .with_label_lookup(Ok(Some("/dev/disk/by-label/LOFTD_NIX")));

    let path = find_labeled_disk_with_probe("LOFTD_NIX", "loftd-nix", &["/dev/vda"], &probe)
        .expect("label lookup fallback should be returned");

    assert_eq!(path, PathBuf::from("/dev/disk/by-label/LOFTD_NIX"));
    assert_eq!(
        probe.calls(),
        vec![
            ProbeCall::CandidateLabel("/dev/vda".to_owned()),
            ProbeCall::LookupLabel("LOFTD_NIX".to_owned())
        ]
    );
}
...
fn preferred_candidate_without_label_falls_back_to_enumeration() {
    let probe = FakeDiskProbe::default()
        ...
        .with_enumeration("/dev/disk/by-id/*loftd-nix*", &[])
        .with_enumeration("/dev/vd?", &["/dev/vdb", "/dev/vdc"])
        .with_candidate_label("/dev/vdb", Ok(Some("OTHER_LABEL")))
        .with_candidate_label("/dev/vdc", Ok(Some("LOFTD_NIX")));
    ...
    assert_eq!(
        ...
        vec![
            ProbeCall::CandidateLabel("/dev/vda".to_owned()),
            ProbeCall::LookupLabel("LOFTD_NIX".to_owned()),
            ProbeCall::EnumerateCandidates("/dev/disk/by-id/*loftd-nix*".to_owned()),
...
fn preferred_candidate_blkid_execution_error_is_not_silently_ignored() {
    ...
    let error = find_labeled_disk_with_probe("LOFTD_NIX", "loftd-nix", &["/dev/vda"], &probe)
...
fn compatibility_entrypoint_starts_with_blkid_label_lookup_when_no_preferred_candidates() {
    let probe =
        FakeDiskProbe::default().with_label_lookup(Ok(Some("/dev/disk/by-label/LOFTD_NIX")));

    let path = find_labeled_disk_with_probe("LOFTD_NIX", "loftd-nix", &[], &probe)
        .expect("label lookup should be returned");

    assert_eq!(path, PathBuf::from("/dev/disk/by-label/LOFTD_NIX"));
    assert_eq!(
        probe.calls(),
        vec![ProbeCall::LookupLabel("LOFTD_NIX".to_owned())]
    );
}

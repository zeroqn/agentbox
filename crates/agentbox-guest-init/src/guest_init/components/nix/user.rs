...
fn reject_failed_or_missing(status_path: &Path, status: &NixPrepStatus) -> Result<()> {
    match status.state {
        ...
        NixPrepState::NotStarted => bail!(
            "libkrun /nix overlay prep has not started; missing status at {}",
            status_path.display()
        ),
...
fn ensure_running_pid_is_live(
    ...
    bail!(
        "libkrun /nix overlay prep has stale/dead PID {pid}; status={}{}",
        status_path.display(),
        log_suffix(status)
    );
}
...
fn failed_message(status_path: &Path, status: &NixPrepStatus) -> String {
    ...
    format!(
        "libkrun /nix overlay prep failed: {error}; status={}{}",
        status_path.display(),
        log_suffix(status)
    )
}
...
fn socket_timeout_message(
    ...
    format!(
        "timed out after {}s waiting for libkrun nix-daemon Unix socket at {} \
         (observed={}); status={}{}",
        timeout.as_secs(),
        socket_path.display(),
        socket_state.as_str(),

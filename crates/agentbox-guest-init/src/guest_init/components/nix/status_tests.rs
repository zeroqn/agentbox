...
fn nix_status_allows_legal_transitions() {
    let running = NixPrepStatus::running(42, PathBuf::from("/run/agentbox/nix.log"));
...
fn nix_status_round_trips_failed_with_log_path() {
    let running = NixPrepStatus::running(7, PathBuf::from("/run/agentbox/nix-prep.log"));
    ...
    assert_eq!(
        parsed.log_path,
        Some(PathBuf::from("/run/agentbox/nix-prep.log"))
    );
...
fn nix_status_running_write_does_not_overwrite_terminal_state() {
    ...
    let stale_running = NixPrepStatus::running(8, PathBuf::from("/run/agentbox/nix-prep.log"));
...
fn nix_wait_timeout_message_includes_status_and_log() {
    let status = NixPrepStatus::running(99, PathBuf::from("/run/agentbox/nix-prep.log"));
    let message = format_wait_timeout(
        Path::new("/run/agentbox/nix-prep.status"),
        &status,
        Duration::from_secs(120),
    );
    assert!(message.contains("timed out after 120s"));
    assert!(message.contains("/run/agentbox/nix-prep.status"));
    assert!(message.contains("/run/agentbox/nix-prep.log"));
}

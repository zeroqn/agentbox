...
pub(in crate::guest_init) fn run(target: ResizeTarget) -> Result<()> {
    let env = LibkrunEnv::from_process_env()?;
    run_with_ops(target, &env, &SystemResizeOps)
}

fn run_with_ops(target: ResizeTarget, env: &LibkrunEnv, ops: &impl ResizeOps) -> Result<()> {
    ...
    if !was_mounted {
        ops.mount_btrfs(&disk, mount)
            .with_context(|| format!("failed to mount libkrun {} btrfs disk", target.name()))?;
    }
    ops.resize_max(mount).with_context(|| {
        format!(
            "failed to resize libkrun {} btrfs filesystem",
            target.name()
        )
    })?;
    if !was_mounted {
        ops.unmount(mount)
            .with_context(|| format!("failed to unmount libkrun {} resize mount", target.name()))?;
    }
    Ok(())
}

trait ResizeOps {
    fn find_disk(&self, target: ResizeTarget, env: &LibkrunEnv) -> Result<PathBuf>;
...
impl ResizeTarget {
    ...
    fn mount_point(self) -> &'static Path {
        match self {
            Self::Nix => Path::new("/run/agentbox/resize-nix"),
            Self::Containers => Path::new("/run/agentbox/resize-containers"),
        }
    }
}
...
mod tests {
    ...
    fn resize_mounts_resizes_and_unmounts_selected_nix_disk() {
        ...
        assert_eq!(
            ...
            [
                "find:nix",
                "mkdir:/run/agentbox/resize-nix",
                "findmnt:/run/agentbox/resize-nix",
                "mount:/dev/vda:/run/agentbox/resize-nix",
                "resize:/run/agentbox/resize-nix",
                "umount:/run/agentbox/resize-nix",
            ]
        );
    }
    ...
    fn resize_reuses_pre_mounted_private_path_without_unmounting() {
        ...
        assert_eq!(
            ...
            [
                "find:containers",
                "mkdir:/run/agentbox/resize-containers",
                "findmnt:/run/agentbox/resize-containers",
                "resize:/run/agentbox/resize-containers",
            ]
        );
    }
    ...
    impl ResizeOps for FakeResizeOps {
        fn find_disk(&self, target: ResizeTarget, _env: &LibkrunEnv) -> Result<PathBuf> {
            match target {
                ResizeTarget::Nix => {

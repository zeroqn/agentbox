...
pub(in crate::guest_init) const MIMALLOC_LIB_ENV: &str = "AGENTBOX_MIMALLOC_LIB";
pub(in crate::guest_init) const GRAPHENE_HARDENED_MALLOC_LIB_ENV: &str =
    "AGENTBOX_GRAPHENE_HARDENED_MALLOC_LIB";
pub(in crate::guest_init) const NIX_ALLOCATOR_ENV: &str = "AGENTBOX_NIX_ALLOCATOR";
...
fn allocator_kind_from_env() -> Result<AllocatorKind> {
    match std::env::var(NIX_ALLOCATOR_ENV) {
        ...
        Err(std::env::VarError::NotUnicode(_)) => {
            bail!("AGENTBOX_NIX_ALLOCATOR must be valid UTF-8")
        }
    }
}
...
fn parse_allocator_kind(value: &str) -> Result<AllocatorKind> {
    match value {
        ...
        _ => bail!("AGENTBOX_NIX_ALLOCATOR must be 'mimalloc' or 'hardened'"),
    }
}
...
fn select_allocator_lib(
    ...
    if kind == AllocatorKind::Hardened {
        bail!(
            "AGENTBOX_NIX_ALLOCATOR=hardened requires {} in {ALLOCATOR_METADATA_PATH} or AGENTBOX_GRAPHENE_HARDENED_MALLOC_LIB",
            kind.metadata_key()
        );
    }

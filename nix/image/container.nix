{ pkgs, pkgsMaster, ohMyCodex, opencode, rtkPrebuilt, agentboxMuslPackage }:
let
  ...
  layers = import ./layers.nix {
    inherit pkgs pkgsMaster ohMyCodex opencode rtkPrebuilt agentboxMuslPackage entrypoint;
    fishConfig = configPayloads.fishConfig;
  };
in

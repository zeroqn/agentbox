...
fn resolve_libkrun_ram_mib_from_meminfo(
    ...
    let ram_mib = match explicit_mem_gib {
        Some(gib) => mem_gib_to_mib(gib)?,
        None => default_libkrun_ram_mib_from_meminfo(meminfo)?,
    };
...
mod tests {
    ...
    #[test]
    fn resolve_libkrun_ram_mib_defaults_to_eighty_percent_host_meminfo() {
        let meminfo = "MemTotal:       10485760 kB
";
        assert_eq!(
...
",
            ...
            .expect("native without --mem should be valid"),
            None
        );
    }
}

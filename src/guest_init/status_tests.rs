...
fn podman_status_allows_legal_transitions() {
    let running = PodmanPrepStatus::running(42, PathBuf::from("/run/agentbox/podman.log"));
...
fn podman_status_round_trips_failed_with_log_path() {
    let running = PodmanPrepStatus::running(7, PathBuf::from("/run/agentbox/podman-prep.log"));
    ...
    assert_eq!(
        parsed.log_path,
        Some(PathBuf::from("/run/agentbox/podman-prep.log"))
    );
...
fn podman_status_running_write_does_not_overwrite_terminal_state() {
    ...
    let stale_running =
        PodmanPrepStatus::running(8, PathBuf::from("/run/agentbox/podman-prep.log"));
...
fn podman_wait_timeout_message_includes_status_and_log() {
    let status = PodmanPrepStatus::running(99, PathBuf::from("/run/agentbox/podman-prep.log"));
    let message = format_wait_timeout(
        Path::new("/run/agentbox/podman-prep.status"),
        &status,
        Duration::from_secs(120),
    );
    assert!(message.contains("timed out after 120s"));
    assert!(message.contains("/run/agentbox/podman-prep.status"));

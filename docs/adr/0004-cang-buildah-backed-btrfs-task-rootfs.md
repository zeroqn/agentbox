# Cang digest-keyed btrfs image-source cache

Status: accepted (revises the original Buildah-per-task btrfs decision)

Cang's default `btrfs-snapshot` task rootfs path uses Buildah as the authority
for OCI image resolution, local image presence, pulling, and freshness. For
same-digest restarts, cang also maintains a digest-keyed btrfs image-source
...
This revises the earlier ADR 0004 position that rejected a cang-owned extracted
...
The earlier cang plan preferred snapshotting directly from a Buildah-mounted
...
- default image selection remains local-first: inspect `localhost/cang:latest`
  locally and use it with `--pull=never` when present;
- if the local image is missing, use `ghcr.io/zeroqn/cang:latest` with
  `--pull=missing`;
- `--pull-latest` refreshes `ghcr.io/zeroqn/cang:latest` through Buildah before
  cache lookup;
...
- cache entries are keyed only by known resolved digest, using filesystem-safe
  digest keys under cang's per-user image state directory;
...
- cache misses use the existing Buildah materializer transaction to validate
  exactly one executable `cang-guest-init` and materialize the task rootfs;
...
entries may accumulate under cang image state until a future cache-management
slice.

A machine whose Buildah storage, cang task state, or cang image cache directory
...
- Keep a `reflink` backend in cang v1: rejected by the task-rootfs backend
  policy.

# Extract direct-libkrun microvm into loftd and loftd-init

Status: accepted

Agentbox already contains an experimental direct-libkrun `microvm` path, but the long-term owner will be separate workspace crates named `loftd` and `loftd-init` so the microvm runtime can have a clean identity, state root, image contract, and host dependency boundary without inheriting agentbox's Podman-backed runtime selection.
...
`loftd-init` will preserve the guest bootstrap role and rootless Podman preparation inside the VM, but it will use canonical loftd naming and will not accept legacy `AGENTBOX_*` aliases. Existing `agentbox microvm` code stays in place until `loftd` and `loftd-init` are complete; the first extraction slice creates compiling structure-preserving skeleton crates rather than bulk-copying a renamed runtime.
...
- Accept `AGENTBOX_*` aliases in loftd-init: rejected because the new guest contract should be canonical from the start.

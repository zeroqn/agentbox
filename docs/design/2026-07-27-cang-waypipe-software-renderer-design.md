# Cang Waypipe Software Renderer Design
...
`cang --waypipe` deliberately starts the guest Waypipe server with `--no-gpu` and does not enable libkrun virtio-GPU. Applications that can render directly into Wayland shared-memory buffers, such as Foot, work. Applications that require a graphics API fail because the cang guest image contains graphics loaders but no Mesa software drivers:
...
- Allow OpenGL/EGL applications such as Ghostty to launch through `cang --waypipe` using llvmpipe.
- Allow Vulkan applications such as Rio to launch through `cang --waypipe` using lavapipe.
...
- Add Ghostty itself to the cang image.
- Add Ghostty- or Rio-specific runtime branches.
- Globally force software rendering for all cang guest commands.
...
The cang image provides a stable guest path that points to the complete Mesa package. Guest-init references that path only when wrapping a command with Waypipe.
...
### Cang-only image contents

Add `pkgs.mesa` to the cang-only package list in `nix/image/layers.nix`. The agentbox image remains unchanged.

During cang image assembly in `nix/image/container.nix`, create this stable link:

```text
/usr/lib/cang-software-renderer -> ${pkgs.mesa}
```
...
```text
/usr/lib/cang-software-renderer/lib/dri/swrast_dri.so
/usr/lib/cang-software-renderer/lib/libEGL_mesa.so.0
/usr/lib/cang-software-renderer/share/glvnd/egl_vendor.d/50_mesa.json
/usr/lib/cang-software-renderer/lib/libvulkan_lvp.so
/usr/lib/cang-software-renderer/share/vulkan/icd.d/lvp_icd.x86_64.json
```

### Guest Waypipe environment

When `CANG_WAYPIPE_PORT` is present, guest-init constructs the Waypipe launch with these child-process environment variables:
...
- The user invokes `cang --waypipe=SOCKET -- COMMAND`.
- Cang validates the forwarded Unix socket and maps it to a guest vsock port.
- Cang does not enable `GpuMode::Drm` for this launch.
- Guest-init sees `CANG_WAYPIPE_PORT` and constructs the software-renderer environment.
...
Guest-init does not probe for Mesa files and does not select an application-specific fallback. The cang image contract guarantees that the stable renderer tree and required artifacts exist.
...
Extend `nix/image/checks.nix` for the cang variant to assert:

- `pkgs.mesa` is rooted by the cang image definition.
- The image assembly creates `/usr/lib/cang-software-renderer`.
...
The agentbox variant must remain free of this cang-only renderer contract.
...
Also build or run the cang image checks so the filesystem and closure assertions are evaluated.
...
```bash
cang --workspace=/absolute/workspace --waypipe=/absolute/forwarded.sock -- ghostty
cang --workspace=/absolute/workspace --waypipe=/absolute/forwarded.sock -- rio
```

# Unified cang guest permissions design
...
```bash
cang --permissions=io-uring,net-admin,bpf,perf
```
...
No permission is enabled by default. The selected Linux capabilities apply to all guest `dev` workload paths for the task, including the initial command, managed PTY command, hidden `as-dev` transition, and later `cang exec` commands. They exist only inside the guest VM and do not grant capabilities to host processes or host namespaces.
...
- Add `CAP_PERFMON` or arbitrary Linux capability names. `CAP_SYS_ADMIN` is available only
  through the explicit independent `sys-admin` permission and `cang-granted`.
...
```bash
cang --permissions=io-uring
cang --permissions=net-admin,bpf
cang --permissions=io-uring,net-admin,bpf,perf
```
...
grant gives commands launched through `cang-granted` `CAP_SYS_ADMIN`, which satisfies
...
`cang-granted`. This capability is exceptionally broad and remains absent from ordinary
...
Setting file capabilities on selected executables is rejected because cang launches arbitrary commands and interpreters. It would require predicting which binaries need privilege, mutate the task filesystem, and behave poorly for dynamically installed programs.

#### Running the workload as root

Running privileged workloads as guest root is rejected because it grants substantially more authority than requested and conflicts with cang's normal `dev` workload model.

#### Initial-command-only capabilities

Granting capabilities only to the initial command is rejected because later `cang exec` commands would not honor the task's launch permission contract.

## Workload scope

The selected Linux capabilities apply to all processes that cang launches as guest user `dev` for the task:
...
- Every later non-PTY foreground command launched through `cang exec`.
- Ordinary descendants of those processes, subject to normal Linux capability and executable-transition rules.

The permission set is task-scoped and immutable after launch. `cang exec` does not accept a separate permission override.

A workload explicitly launched with cang's existing root mode remains guest root and does not use the `dev` capability transition. The `io-uring` and `perf` selections still control their guest-wide kernel policy changes. This design does not attempt to reduce guest root to only the selected capabilities.
...
```text
CANG_PERMISSIONS=io-uring,net-admin,bpf,perf
```
...
The previous `CANG_IO_URING` and `CANG_PERF` guest markers are removed together with the old CLI contract. No compatibility decoding is required.

## Guest-init architecture

Guest-init parses `CANG_PERMISSIONS` into its own typed permission set and validates every token again. It does not trust host-side validation as the sole boundary check.
...
- `CANG_PERMISSIONS` is an internal host-to-guest launch contract, not a workload-controlled authorization API.
...
- `CANG_PERMISSIONS` contains an unknown or empty token.
...
- Guest configuration emits one canonical `CANG_PERMISSIONS` value.
...
- Old `CANG_IO_URING` and `CANG_PERF` markers are no longer emitted.

### Guest parsing and policy tests

- Missing `CANG_PERMISSIONS` parses as an empty set.
...
When the environment permits a cang live smoke test:
...
- Confirm a later `cang exec` command observes the same selected capability set.
...
- `cang --permissions=io-uring,net-admin,bpf,perf` parses and reaches guest-init as one validated permission set.
...
- Later `cang exec` processes receive the task's selected capability set.

# cang `--gpu=drm` → Venus Vulkan via standalone sandboxed render-server runner
...
`cang --gpu=drm` currently configures the virtio-gpu with the native-context GL
...
### 1. `deps/libkrun` (submodule, cang fork — left as a submodule)
...
cang fork at `ad8a404`). libkrunfw stays prebuilt. This is the heaviest lift:
...
### 3. `crates/cang/src/runtime/vm/gpu.rs`
...
### 4. `crates/cang/src/runtime/vm/libkrun/launcher.rs`
...
### 5. `crates/cang/src/runtime/vm/libkrun/dynamic.rs`

Load `krun_set_gpu_options3` as an optional symbol (same pattern as
`set_gpu_options2`); error if unavailable.

### 6. New: render-server runner (under `crates/cang/src/runtime/session/supervisor/`)
...
- The supervisor keeps the parent end, passes its fd number to the VM worker
  via a new env var (`CANG_RENDER_SERVER_FD=N`); the fd survives the helper
...
### 7. `crates/cang/src/runtime/session/supervisor/command.rs` (`helper_env`)
...
`CANG_RENDER_SERVER_FD` number.

### 8. `crates/cang/src/runtime/landlock.rs`
...
### 9. `crates/cang/assets/seccomp/render-server.json`
...
empirically with cang's existing seccomp audit mode, then hand-trimmed.

### 10. `crates/cang/src/runtime/launch/config/mod.rs`
...
- Use cang seccomp audit mode to iterate the runner's syscall set empirically.

# Cang Private Host-Loopback Pulse Bridge Design
...
Cang currently treats `--pulse=tcp:IP:PORT` as a literal PulseAudio-compatible TCP endpoint and exports it to the guest as `PULSE_SERVER`. This works when the endpoint is reachable through the guest network, but `tcp:127.0.0.1:PORT` points at the guest loopback interface rather than a host service bound to host loopback.
...
The direct endpoint form remains literal. Cang does not resolve arbitrary hostnames or rewrite non-loopback addresses.
...
```text
guest Pulse client
PULSE_SERVER=unix:/run/user/<uid>/cang-pulse
        |
        v
guest Unix listener
...
dedicated task vsock port
        ...
        v
private host Unix socket under /tmp/cang-<uid>/
...
The bridge uses the existing owner-only per-user cang runtime directory under `/tmp/cang-<uid>/`. Socket allocation follows the same path-length, stale-socket, ownership, and mode checks used by managed attach, exec, and Waypipe sockets.
...
- Bind `/run/user/<uid>/cang-pulse` as a Unix stream listener.
...
Existing tasks retain the endpoint selected at launch. `cang exec` inherits either the direct endpoint or bridge configured for that task and cannot retarget it.
...
- `crates/cang/src/runtime/launch/config/model.rs`
- `crates/cang/src/runtime/launch/config/codec.rs`
- `crates/cang/src/runtime/launch/config/guest_env.rs`
- `crates/cang/src/runtime/launch/plan.rs`
- `crates/cang/src/runtime/session/managed_attach_socket.rs`
- `crates/cang/src/runtime/session/supervisor/entry.rs`
- `crates/cang/src/runtime/vm/libkrun/launcher.rs`
- A focused Pulse bridge module under `crates/cang/src/runtime/session/`

Guest crate:

- Guest launch-contract parsing under `crates/cang-guest-init/src/guest_init/runtime/`
- Shell environment construction under `crates/cang-guest-init/src/guest_init/components/shell/`
- A focused Pulse bridge component under `crates/cang-guest-init/src/guest_init/components/`
...
Run a host Pulse-compatible listener bound only to `127.0.0.1:4714`, then launch cang with `--pulse=tcp:localhost:4714` and verify guest audio playback. Confirm that:

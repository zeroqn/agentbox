# cang Pulse TCP passthrough design
...
Add an opt-in `cang --pulse=tcp:IP:PORT` launch option that directs PulseAudio-compatible applications inside a new cang guest to a host-provided Pulse TCP server.

The first version uses a direct client connection. Cang does not proxy the Pulse protocol, create a guest-local Pulse Unix socket, or start PipeWire services inside the guest. The host is responsible for running PipeWire with `pipewire-pulse`, exposing an authenticated or ACL-restricted TCP listener, and making the supplied address reachable from the selected cang network mode.
...
- Changing audio settings for already-running tasks through `cang exec` or `cang attach`.
...
```bash
cang --pulse=tcp:192.168.1.10:4713
cang --tsi --pulse=tcp:127.0.0.1:4713
cang --pulse=tcp:[2001:db8::10]:4713 -- paplay sample.wav
```
...
The supplied address is used literally. Cang does not rewrite loopback addresses, discover a host address, or select a PipeWire listener automatically.
...
The launch configuration exposes the endpoint to guest-init through an internal `CANG_PULSE_SERVER` value. The host does not broadly copy `PULSE_SERVER` from its own environment.
...
Guest-init reads the optional `CANG_PULSE_SERVER` contract during normal cang entry setup. When present, it validates the same `tcp:IP:PORT` shape and adds the canonical value to the final guest environment as:
...
Cang documentation will make clear that a TCP Pulse listener can expose playback, capture, and other Pulse server operations to connecting clients. The host configuration must restrict access using PipeWire-Pulse access controls, network isolation, or another host-managed policy. Cang does not enable anonymous access itself.
...
Cang performs syntax validation only. Connection failures remain normal Pulse client errors inside the guest.

## Data flow

- The user starts `cang --pulse=tcp:IP:PORT`.
- Clap invokes the dedicated endpoint parser.
- Cang stores the validated endpoint in the runtime and launch models.
- Launch configuration serialization preserves the optional endpoint.
- Launch configuration construction emits `CANG_PULSE_SERVER=tcp:IP:PORT` for guest-init.
...
Cang fails before VM startup when:
...
Cang does not fail launch because:
...
The design therefore keeps the capability explicit and disabled by default. Cang does not automatically expose host PipeWire on a wildcard address, alter firewall rules, enable anonymous authentication, or forward credentials.

A loopback-only listener with TSI is the recommended local configuration when it works for the host's libkrun setup. For passt, users must deliberately select a guest-reachable address and restrict it outside cang.
...
- Guest configuration emits the exact canonical `CANG_PULSE_SERVER` value only when enabled.
...
- Launch a current cang and matching guest-init with `--pulse`.
...
- `crates/cang/src/cli/mod.rs`
- cang launch plan and launch configuration model/codec code under `crates/cang/src/runtime/launch/`
- `crates/cang-guest-init/src/guest_init/runtime/cang.rs` and focused environment tests
- `README.md`

# Explicit Cang Capability Grant Helper

## Summary

Cang currently grants capability-bearing guest permissions to the normal `dev` workload process tree. This makes every descendant inherit capabilities, including Bubblewrap, which deliberately refuses non-root, non-setuid startup with a nonempty permitted capability set.

Replace task-wide process capability propagation with an explicit command boundary:

```sh
cang-granted COMMAND [ARG...]
```

Normal workloads remain capability-free. When a task is launched with capability-bearing `--new-perms` values, guest-init installs a dedicated `cang-granted` file-capability helper containing exactly those authorized capabilities. Invoking the helper grants all authorized capabilities to the requested command.
...
- Make `net-admin`, `net-raw`, and `bpf` available only through explicit `cang-granted` invocation.
- Make the helper's kernel-enforced file-capability metadata the sole runtime source of capability authority.
- Protect all binaries under `/run/cang/wrappers` with a read-only bind mount after initialization.
...
- Making `cang-granted` a sandbox between commands run by the same `dev` user.
...
The existing `cang-guest-init` binary must not become the privileged helper. It exposes multiple initialization and internal subcommands; assigning capabilities to that multi-command binary would unnecessarily enlarge the privileged interface.
...
- `net-admin`: authorizes `CAP_NET_ADMIN` for `cang-granted`.
- `net-raw`: authorizes `CAP_NET_RAW` for `cang-granted`.
- `bpf`: authorizes `CAP_BPF` for `cang-granted`.

Each `cang-granted COMMAND` invocation receives all capability-bearing permissions authorized for the task. The helper accepts no capability selector.

If no capability-bearing permission was authorized, `cang-granted` refuses to execute the command with a clear error.
...
6. Installs a dedicated static `cang-granted` executable at `/run/cang/wrappers/bin/cang-granted` as `root:root`, mode `0555`.
...
10. Bind-mounts `/run/cang/wrappers` onto itself and remounts it read-only.
...
```text
/run/cang/wrappers             root:root, read-only bind mount
/run/cang/wrappers/bin         root:root, 0755
/run/cang/wrappers/bin/newuidmap   root:root, 4755
/run/cang/wrappers/bin/newgidmap   root:root, 4755
/run/cang/wrappers/bin/cang-granted root:root, 0555, exact authorized file capabilities
```
...
The `security.capability` xattr on the root-owned, read-only `cang-granted` executable is the sole runtime source of truth.
...
On `execve()` of `cang-granted`, the kernel derives the helper's capability sets from the executable xattr, subject to the bounding set. The helper then:
...
A command invoked through `cang-granted` intentionally starts a capability-bearing subtree. Descendants inherit according to normal Linux capability and `execve()` rules. Bubblewrap invoked inside that subtree will still reject the inherited permitted capabilities unless they are deliberately dropped first. Bubblewrap invoked from the normal workload tree will start capability-free and should work normally.
...
`cang-granted` fails without executing the target when:
...
Behavioral tests belong under `crates/cang-guest-init/src/` and cover:
...
- `cang-granted fish` receives `CAP_NET_ADMIN` and no unauthorized capabilities.
- `cang-granted` refuses execution in a task without capability-bearing grants.
- `/run/cang/wrappers` rejects writes while all three wrappers remain executable.
